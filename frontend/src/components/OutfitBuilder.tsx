...
function GarmentThumb({
  ...
  const url = garment.processed_url ? getFileUrl(garment.processed_url) : "";
  return (
...
export default function OutfitBuilder({
  ...
  const handleSave = async () => {
    ...
    setSaveError(null);
    try {
      const saved = await saveOutfit(lastJobId, nameInput || "Untitled look");
      addSavedOutfit({
        id: saved.id,
      ...
      setSharedUrl(`${origin}/o/${saved.share_id}`);
      setSavingName(false);
      setNameInput("");
    } catch (e) {
        ...
        <div className="pt-3 border-t border-neutral-100 space-y-2">
          {savingName ? (
            <>
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  className="py-2 px-3 text-sm text-neutral-600 hover:text-neutral-900"
                >
                  Cancel
                </button>
              </div>
            ...
            <button
              ...
              className="w-full py-2 text-sm font-medium text-neutral-700 hover:text-neutral-900"
            >
              Save to lookbook
            </button>
          )}
          {sharedUrl && (
            <div className="text-xs text-neutral-600 break-all bg-neutral-50 p-2 rounded-lg">
              Saved. Share:{" "}
              <button
                onClick={() => {
